...
parent circuit.
"""
function mutate(circuit,terminals = "LRC")
...
function CircuitFitness(circuit,measurements,frequencies)
    CircFunc = CircuitFunction(circuit.circuit)
    ObjFunc = ObjectiveFunction(CircFunc,measurements,frequencies)
    OptParams = OptimizeParameters(ObjFunc,circuit.parameters[circuit.parameter_indices])
    return OptParams, ObjFunc(OptParams)
end
